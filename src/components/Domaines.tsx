...
export default function Domaines() {
  return (
    <section id="domaines" style={{ background: '#1e3829', padding: '7rem 0' }}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div style={{ textAlign: 'center', marginBottom: '4.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '1.2rem' }}>
            ...
            <span style={{ fontSize: '0.58rem', letterSpacing: '0.25em', color: 'rgba(46,90,62,0.90)', textTransform: 'uppercase' }}>Domaines d&apos;intervention</span>
            <div style={{ width: '24px', height: '1px', background: '#2e5a3e' }} />
          </div>
          <h2 style={{ fontFamily: 'var(--font-playfair)', fontSize: 'clamp(1.7rem, 3vw, 2.4rem)', fontWeight: 400, color: '#f3f0e8', margin: 0, lineHeight: 1.20 }}>
            Droit des affaires & immobilier d&apos;entreprise
          </h2>
        </div>

        {/* Featured */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, marginBottom: '1.5rem', background: '#284d36', border: '1px solid rgba(46,90,62,0.20)' }}>
          <div style={{ padding: '3rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
              <Image src={FEATURED.svg} alt="" width={28} height={28} style={{ opacity: 0.70, filter: 'invert(1)' }} />
              <span style={{ fontSize: '0.58rem', letterSpacing: '0.22em', color: 'rgba(46,90,62,0.90)', textTransform: 'uppercase' }}>Compétence principale</span>
            </div>
            <h3 style={{ fontFamily: 'var(--font-playfair)', fontSize: '1.6rem', fontWeight: 400, color: '#f3f0e8', margin: '0 0 1.2rem', lineHeight: 1.25 }}>
              {FEATURED.titre}
            </h3>
            <p style={{ fontSize: '0.84rem', color: 'rgba(243,240,232,0.55)', lineHeight: 1.80 }}>
              {FEATURED.desc}
            </p>
          </div>
          <div style={{ padding: '3rem', borderLeft: '1px solid rgba(46,90,62,0.15)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '1rem' }}>
            {FEATURED.points.map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.9rem' }}>
                <div style={{ width: '20px', height: '1px', background: 'rgba(46,90,62,0.50)', flexShrink: 0 }} />
                <span style={{ fontSize: '0.82rem', color: 'rgba(243,240,232,0.65)' }}>{p}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: 'rgba(46,90,62,0.15)' }}>
          {DOMAINES.map((d) => (
            <div key={d.titre} className="card-hover" style={{ background: '#f3f0e8', padding: '2rem 1.6rem' }}>
              ...
              <p style={{ fontSize: '0.76rem', color: '#5a5040', lineHeight: 1.72 }}>{d.desc}</p>
            </div>
          ))}
        </div>
